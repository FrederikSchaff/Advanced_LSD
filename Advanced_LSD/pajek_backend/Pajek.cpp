...
bool pajek_vertices_add(int ID, char const label[],  int shape, char const *colour, double x_fact, double y_fact, double value, double x_pos, double y_pos, int start, int end )
{
    ...
    PAJEK_MSG("\nPajek Module :  ERROR! Maximum number of vertices reached");
		return false;
	}
...
bool pajek_arcs_add(bool isedge, int source, int target, double value, char const *label,  int width, char const *colour, int start, int end )
{
	if (pajek_arcs_edges_count == PAJEK_MAX_ARCS) {
		PAJEK_MSG("\nPajek Module :  ERROR! Maximum number of Arcs reached");
		return false;
	}
  ...
  if (pajek_init_KindsOfRelation_check || pajek_edges_kindsOf_count>0 || pajek_arcs_kindsOf_count>0){
    if (max(pajek_arcs_kindsOf_count,pajek_edges_kindsOf_count)< PAJEK_KINDS_RELATIONS){
    	...
    	return true;
  		...
  		PAJEK_MSG("\nPajek Module :  Errror! Increase the #define PAJEK_KINDS_RELATIONS.\n");
  		return false;
  	}
  } else {
    PAJEK_MSG("\nPajek Module :  Error! First initialise pajek via pajek_init()!");
    return false;
  }
}
...
bool pajek_init( int serial, bool append, char const *dirname_suffix, char const *filename_suffix )
{
	if (!append && PAJEK_FORCE_COMPLETE){
		PAJEK_MSG("\nPajek Module :   Error! Append (paj project file) mode necessary in" \
...
bool pajek_openFile(bool append, int serial, int time)
{
  snprintf(pajek_dir_path,sizeof(char)*64,"%s%s%s",\
	...
	if (!append){
		if (time == -1){
			PAJEK_MSG("\nPajek Module :  Error: No time provided for non-append call of pajek_openFile");
			return false;
		}
   ...
   snprintf(pajek_path,sizeof(char)*300,"%s%s%s_%s_s%d.%s", \
    ...
    PAJEK_MSG("\nPajek Module :  Path of (temporary) .paj file is:\n\t");
	...
	if (!pajek_file.is_open()){
  	pajek_file.open(pajek_path,std::ios_base::out | std::ios_base::trunc);
		if (pajek_file.is_open()){
			...
			PAJEK_MSG("\nPajek Module :  pajek_file could not be opened. Check path.");
			return false;
		}
	} else {
		PAJEK_MSG("\nPajek Module :  pajek_file is already open!.");
		return false;
	}
}
/* Close the file */
bool pajek_close(bool final, bool append)
{
	if (pajek_file.is_open()){
      ...
      return true;
		...
		PAJEK_MSG("\nPajek Module :  pajek_file could not be closed for it was not open.");
		return false;
	}
}
...
bool pajek_write(int time)
{
  ...
  if (time == -1){
	...
	if (!pajek_openFile(false,pajek_serial,pajek_time)){
		PAJEK_MSG("\nPajek Module :  Could not open file");
  	return false;
	}

	if (!pajek_append(pajek_time)){
  	PAJEK_MSG("\nPajek Module :  Could not append file");
    return false;
	}
	if (!pajek_close()){
    PAJEK_MSG("\nPajek Module :  Could not close file");
    return false;
	}
...
bool pajek_append (int time, bool final )
{
  if (pajek_append_mode){
 	...
 	if (!pajek_file.is_open()){
		PAJEK_MSG("\nPajek Module :  pajek_file is not open!");
		return false;
 	}
 ...
 if (time==-1){
  ...
  pajek_file << "*Vertices " << pajek_vertices_count << "\n";
    ...
    PAJEK_MSG("\nPajek Module :  No Edges or Arcs! (Leads to errors with PajekToSvgAnim)");
	}
	/*
		*Arcs :1 "relation name"
		 init_vertex term_vertex value w width c color [activ_int]
  ...
  pajek_file << "\n";
    ...
    pajek_write(pajek_time); //Additional writing of file.
		PAJEK_MSG("\nPajek Module :  Final network saved to:\n\t");
		PAJEK_MSG(pajek_path);
	}
...
{
	PAJEK_MSG("\nPajek Module :  Saving partial project file.");
  if (pajek_file.is_open()){
  	pajek_file.close();
	} else {
    PAJEK_MSG("\nPajek Module :  pajek_file could not be closed (here in pajek_partial_snaps).");
		return false;
	}
	char rename[300];
  ...
  snprintf(rename,sizeof(char)*300,"%s%s%s_%s_s%d", \
	...
	if(std::remove(rename)==0){
		PAJEK_MSG("\nPajek Module :  Deleted old file with same name.");
	}
	if(std::rename(pajek_path, rename)!=0) {
		PAJEK_MSG("\nPajek Module :  Error renaming");
		return false;
	} else {
    PAJEK_MSG("\nPajek Module :  Saved (renamed) snapshot to: ");
    PAJEK_MSG(rename);
	}
  pajek_snapshot_parts++;
	if (renew){
  	return pajek_openFile(true,pajek_serial);
		...
		PAJEK_MSG("\nPajek Module :  Final pajek file saved, temporary file removed. Saved to:\n\t");
		PAJEK_MSG(pajek_path);
    pajek_file.close();
		return true;
	}
...
{
	if (tau > 1.0 || tau < 0.0){
		PAJEK_MSG("\nPajek Module :  Error in pajek_relative_xy. Invalid value for tau");
		return false;
	}
...
bool pajek_vertices_timeline_add(int ID, char const *label, int snapshot){
	if (snapshot > PAJEK_MAX_SNAPSHOTS){
		PAJEK_MSG("\nPajek Module :  Error! More snapshots than allowed via initialisation (PAJEK_MAX_SNAPSHOTS)");
		return false;
	}
	int i = 0;
	while (			pajek_vertices_timeStamps_id[i]!=-1 \
		...
		if (i == PAJEK_MAX_VERTICES){
			PAJEK_MSG("\nPajek Module :  Error in pajek_vertices_timeline_add");
			return false;
		}
	}
...
bool pajek_arcs_timeline_add(int Source, int Target, char const *Kind, int snapshot, bool isEdge){
  if (snapshot > PAJEK_MAX_SNAPSHOTS){
    ...
    */
		if (i==PAJEK_MAX_ARCS){
			PAJEK_MSG("\nPajek Module :  Error in pajek_arcs_timeline_add.");
			return false;
		}
	}
...
bool pajek_timeline_close(){
  ...
  if (!pajek_file.is_open()){
    ...
    if (!pajek_file.is_open() ){
			PAJEK_MSG("\nPajek Module :  In close_timeline() new output pajek_file could not be opened. Check path.");
			return false;
		}
		pajek_file_in.open(pajek_path,std::ios_base::in);
		if (!pajek_file_in.is_open()){
			PAJEK_MSG("\nPajek Module :  In close_timeline() pajek_file_in could not be opened. Check path.");
			return false;
		}
	} else {
		PAJEK_MSG("\nPajek Module :  In close_timeline() pajek_file is still open!.");
		return false;
	}
